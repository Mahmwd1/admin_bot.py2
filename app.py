...
# الإصدار النهائي - مع إصلاح دقة الوقت وإضافة النسخ الاحتياطي التلقائي
...
ADMIN_BOT_TOKEN = "8846182269:AAHknfx1Mg3-gzyRq67z8BVw7JtiLvVW1ac"
ADMIN_IDS = [8587386123, 8612276675]

# ✅ توكن بوت النسخ الاحتياطي
BACKUP_BOT_TOKEN = "8774063852:AAFBNCRbFlJMwJWDsQudUBtfZy3iC8Smbhk"
BACKUP_BOT_CHAT_ID = 8587386123  # معرف الأدمن
...
# =========================================================
# 🔍 دوال النسخ الاحتياطي - مع إرسال للبوت الرابع
# =========================================================
...
# ✅ دالة إرسال النسخة الاحتياطية للبوت الرابع
async def send_backup_to_backup_bot(context: ContextTypes.DEFAULT_TYPE):
    """إرسال النسخة الاحتياطية للبوت الرابع مع معلومات مفصلة"""
    try:
        # جلب الإحصائيات
        stats = get_stats()
        ...
        # ✅ إرسال للبوت الرابع
        url = f"https://api.telegram.org/bot{BACKUP_BOT_TOKEN}/sendDocument"
        
        with open(zip_filename, 'rb') as f:
            files = {'document': f}
            data = {
                'chat_id': BACKUP_BOT_CHAT_ID,
                'caption': (
                    f"💾 **النسخة الاحتياطية التلقائية**\n\n"
                    f"📅 التاريخ: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                    f"📊 **إحصائيات:**\n"
                    f"• 📦 إجمالي الحسابات: {stats['total_active']}\n"
                    ...
                    f"📌 تم إرسالها تلقائياً كل يومين"
                ),
                'parse_mode': 'Markdown'
            }
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, data=data, files=files) as response:
                    ...
                    if result.get("ok"):
                        logger.info("📤 تم إرسال النسخة الاحتياطية للبوت الرابع بنجاح")
                    else:
                        logger.error(f"❌ فشل إرسال النسخة للبوت الرابع: {result}")
...
# =========================================================
# 🔍 دوال تحليل النصوص - مع استخراج رمز الأمان + دقة الوقت
# =========================================================
...
def parse_recovery_message(text: str) -> Dict:
    """تحليل رسالة الاستعادة مع استخراج رمز الأمان (6 أرقام) ودقة الوقت بالثواني"""
    result = {
        "bound_email": "لا توجد",
        ...
        "security_code": None,
        "total_seconds": 0  # ✅ إضافة المدة بالثواني
    }
    ...
    # ✅ حساب الوقت بدقة بالثواني
    total_seconds = 0
    
    # أنماط الوقت بالثواني والدقائق والساعات والأيام
    time_patterns = [
        (r'(\d+)\s*(?:ثانية|ثواني|second|seconds)', 1),
        (r'(\d+)\s*(?:دقيقة|دقائق|minute|minutes)', 60),
        (r'(\d+)\s*(?:ساعة|ساعات|hour|hours)', 3600),
        (r'(\d+)\s*(?:يوم|أيام|day|days)', 86400),
    ]
    ...
    # إذا لقى وقت محدد
    if total_seconds > 0:
        result["recovery_end_time"] = (datetime.now() + timedelta(seconds=total_seconds)).isoformat()
        result["total_seconds"] = total_seconds
        logger.info(f"⏱️ تم استخراج الوقت: {total_seconds} ثانية ({total_seconds/3600:.1f} ساعات)")
    else:
        # أنماط بديلة (في حال ما اشتغلت الأنماط الأولى)
        time_patterns_alt = [
            r'(\d+)\s*(?:يوم|أيام)',
            r'(\d+)\s*(?:ساعة|ساعات)',
            r'(\d+)\s*(?:دقيقة|دقائق)',
        ]
        for pattern in time_patterns_alt:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
...
async def show_confirmed_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ زر حذف جميع الحسابات المؤكدة
    keyboard.append([InlineKeyboardButton("🗑️ حذف جميع المؤكدة", callback_data="delete_all_confirmed")])
...
# =========================================================
# 🗑️ حذف جميع الحسابات المؤكدة
# =========================================================

async def delete_all_confirmed_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """طلب تأكيد لحذف جميع الحسابات المؤكدة"""
    query = update.callback_query
    await query.answer()
    ...
    keyboard = [
        [InlineKeyboardButton("⚠️ نعم، احذف الكل", callback_data="confirm_delete_all_confirmed")],
        [InlineKeyboardButton("❌ إلغاء", callback_data="menu_confirmed")]
    ]
    
    await query.message.edit_text(
        f"🚨 **تحذير: حذف جميع الحسابات المؤكدة**\n\n"
        f"📦 عدد الحسابات المؤكدة: **{count}**\n\n"
...
async def confirm_delete_all_confirmed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """تنفيذ حذف جميع الحسابات المؤكدة"""
    query = update.callback_query
    await query.answer()
    ...
    cursor.execute("DELETE FROM accounts WHERE recovery_confirmed = 1 AND is_archived = 0 AND is_completed = 0")
    deleted_count = cursor.rowcount
    conn.commit()
    conn.close()
    
    add_log(0, "ALL_CONFIRMED_DELETED", f"تم حذف جميع الحسابات المؤكدة ({deleted_count})")
    
    await query.message.edit_text(
        f"🗑️ **تم حذف جميع الحسابات المؤكدة!**\n\n"
        f"📦 عدد الحسابات المحذوفة: **{deleted_count}**",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 رجوع", callback_data="menu_confirmed")]
        ]),
        parse_mode="Markdown"
...
async def scheduled_notifications(app: Application):
    ...
    # ✅ إرسال نسخة احتياطية للبوت الرابع كل يومين
    try:
        last_backup_time = get_last_notification(0, "AUTO_BACKUP")
        if not last_backup_time or (now - datetime.fromisoformat(last_backup_time)).days >= 2:
            await send_backup_to_backup_bot(app)
            save_notification(0, "AUTO_BACKUP")
            logger.info("📤 تم إرسال النسخة الاحتياطية التلقائية للبوت الرابع")
    except Exception as e:
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ 4. إذا كان النص أرقام فقط - بحث (فقط إذا لم تكن هناك حالة انتظار)
    if text.isdigit():
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "delete_all_confirmed":
        await delete_all_confirmed_start(update, context)
    elif data == "confirm_delete_all_confirmed":
        await confirm_delete_all_confirmed(update, context)
...
def main():
    ...
    logger.info("📤 إرسال النسخ الاحتياطية للبوت الرابع كل يومين")
